...
sudo apt-get -y install build-essential checkinstall git libfaac-dev libgpac-dev \
  libmp3lame-dev libopencore-amrnb-dev libopencore-amrwb-dev librtmp-dev libtheora-dev \
    libvorbis-dev pkg-config texi2html yasm zlib1g-dev libvpx-dev
...
# This allows x264 to accept just about any input that FFmpeg can handle and is
# useful if you want to use x264 directly. See a more detailed explanation of
# what this means.
cd ~/x264
make distclean
./configure --enable-static --prefix=/usr/local
...
# file is completely downloaded. Usage: qt-faststart input.mp4 output.mp4.
cd ~/ffmpeg
